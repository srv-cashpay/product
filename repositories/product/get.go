...
func (r *productRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	...
	find := r.DB.Where("merchant_id = ?", req.MerchantID).Where("status = ?", 1).Limit(req.Limit).Offset(offset).Order(req.Sort)
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Product{}).Where("merchant_id = ?", req.MerchantID).Where("status = ?", 1).Count(&totalRows).Error; errCount != nil {
